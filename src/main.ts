...
interface XpPluginSettings {
    level: number;
    experience: number;
    createdDailyNotes: string[]; // 新增：记录已创建的日记
}
...
export default class XpPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.registerEvent(
            this.app.vault.on('modify', async (file) => {
                if (!(file instanceof TFile) || !this.isDailyNote(file)) return;
    ...
    // 新增：生成日记的唯一标识符
    getDailyNoteKey(file: TFile): string {
    ...
    checkForLevelUp() {
        ...
        while (this.settings.experience >= requiredXp) {
            ...
            new Notice(`Level Up! Now you are ${this.settings.level} level !`);
        }
    }
